...
class Prisoner(MovingSprite):
    def __init__(self, game, species, x, y, LorR):
        ...
        self.original = pygame.Surface(
            (int(40 * self.game.scale), int(40 * self.game.scale))).convert()
        self.original.fill(yellow)
        pygame.draw.circle(
            self.original,
            black,
            (int(20 * self.game.scale), int(20 * self.game.scale)),
            int(20 * self.game.scale))
    ...
    def update(self):
        ...
        if self.LorR == 'left':
            x -= int(21 * self.game.scale)
        else:
            x += int(21 * self.game.scale)

        d = self.game.dist(
            x, y, int(600 * self.game.scale), int(450 * self.game.scale))
        if d < int(85 * self.game.scale):
            angle = (
                ((360.0 / self.game.cell_count) * self.species) - 90) % 360
...
class Guard(MovingSprite):
    def __init__(self, game):
        ...
        self.image = pygame.Surface(
            (int(40 * self.game.scale), int(40 * self.game.scale)))
        ...
        self.destx = int(50 * self.game.scale)
        self.desty = int(50 * self.game.scale)
        self.i = 0
        self.stop = 0
    ...
    def circ(self, x, y):
        distance = self.game.dist(
            int(600 * self.game.scale), int(450 * self.game.scale), x, y)
        angle = math.acos(
            float(x - int(600 * self.game.scale)) / float(distance))
        angle *= 180.0 / math.pi  # because pygame likes degrees
        if y > 450:
            angle = 360 - angle
        angle = (480 - angle) % 360
    ...
    def path(self, i):
        ...
        for j in range(-90, 400, (360 // self.game.cell_count)):
            angle.append(j)
        x = (int(d * math.cos(angle[i] * math.pi / 180.0)) + int(
            600 * self.game.scale))
        y = (int(d * math.sin(angle[i] * math.pi / 180.0)) + int(
            450 * self.game.scale))
        return (x, y)
